...
function stuPassReset($user, $password){
    try{
        $dbh = connectDB();
        $statement = $dbh->prepare("update project_student set StuPass = sha2(:newPassword, 256), FirstLogin=false where StuID = :user");
...
function instPassReset($user, $password){
    try{
        $dbh = connectDB();
        $statement = $dbh->prepare("update project_inst set InstPass = sha2(:newPassword, 256), FirstLogin=false where InstID = :user");
        $statement->bindParam(":newPassword", $password);
        $statement->bindParam(":user", $user);
        $result = $statement->execute();
        $dbh=null;
